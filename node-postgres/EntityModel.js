...
const getTeacherSubjectAttendance = ({ sem, actDay, lect }) => {
    return new Promise(function (resolve, reject) {
        if (sem === "BE-SEM8-C-COMP") {
            pool.query(`select ttid,timing from TTSEM8 where subid = '${lect}' and day = '${actDay}'`, (error, results) => {
                if (error) {
                    reject(error);
                }
                resolve(results.rows);
            ...
            pool.query(`select ttid,timing from TTSEM6 where subid = '${lect}' and day = '${actDay}'`, (error, results) => {
                if (error) {
                    reject(error);
                }
                resolve(results.rows);
            ...
            pool.query(`select ttid, timing from TTSEM4 where subid = '${lect}' and day = '${actDay}'`, (error, results) => {
                if (error) {
                    reject(error);
                }
                resolve(results.rows);
...
const getTeacherSubjectTable = ({ date, time, sem }) => {
    return new Promise(function (resolve, reject) {
        if (sem === 'BE-SEM8-C-COMP') {
            pool.query(`select a.studid , s.studname, a.status from attendance_sem8 a , students s where cldate = '${date}'  and ttid = ${time} and a.studid = s.studid order by a.studid`, (error, results) => {
                if (error) {
                    reject(error);
                }
                resolve(results.rows);
            ...
            pool.query(`select a.studid , s.studname, a.status from attendance_sem6 a , students s where cldate = '${date}'  and ttid = ${time} and a.studid = s.studid order by a.studid`, (error, results) => {
                if (error) {
                    reject(error);
                }
                resolve(results.rows);
            ...
            pool.query(`select a.studid , s.studname, a.status from attendance_sem4 a , students s where cldate = '${date}'  and ttid = ${time} and a.studid = s.studid order by a.studid`, (error, results) => {
                if (error) {
                    reject(error);
                }
                resolve(results.rows);
    ...
    return new Promise(function (resolve, reject) {
        if (sem === 'BE-SEM8-C-COMP') {
            for (let i = 0; i < finalInsert.length; i++) {
                ...
                pool.query(`INSERT INTO ATTENDANCE_SEM8 (STUDID , CLDATE, STATUS , TTID) VALUES ('${studid}' , '${date}' , '${status}' , ${timing});`, (error, results) => {
                    if (error) {
                        reject(error);
                    }
            ...
            for (let i = 0; i < finalInsert.length; i++) {
                ...
                let date = finalInsert[i].date
                let timing = finalInsert[i].timing
                pool.query(`INSERT INTO ATTENDANCE_SEM6 (STUDID , CLDATE, STATUS , TTID) VALUES ('${studid}' , '${date}' , '${status}' , ${timing});`, (error, results) => {
                    if (error) {
                        reject(error);
                    }
                    resolve('students attendance added');
            ...
            for (let i = 0; i < finalInsert.length; i++) {
                ...
                let date = finalInsert[i].date
                let timing = finalInsert[i].timing
                pool.query(`INSERT INTO ATTENDANCE_SEM4 (STUDID , CLDATE, STATUS , TTID) VALUES ('${studid}' , '${date}' , '${status}' , ${timing});`, (error, results) => {
                    if (error) {
                        reject(error);
                    }
                    resolve('students attendance added');
    ...
    return new Promise(function (resolve, reject) {
        if (sem === 'BE-SEM8-C-COMP') {
            for (let i = 0; i < finalInsert.length; i++) {
                ...
                let date = finalInsert[i].date
                let timing = finalInsert[i].timing
                pool.query(`UPDATE ATTENDANCE_SEM8 SET STATUS = '${status}' WHERE studid = '${studid}' and cldate = '${date}' and ttid = ${timing} ;`, (error, results) => {
                    if (error) {
                        reject(error);
                    }
                    resolve('students attendance added');
            ...
            for (let i = 0; i < finalInsert.length; i++) {
                ...
                let date = finalInsert[i].date
                let timing = finalInsert[i].timing
                pool.query(`UPDATE ATTENDANCE_SEM6 SET STATUS = '${status}' WHERE studid = '${studid}' and cldate = '${date}' and ttid = ${timing} ;`, (error, results) => {
                    if (error) {
                        reject(error);
                    }
                    resolve('students attendance added');
            ...
            for (let i = 0; i < finalInsert.length; i++) {
                ...
                let date = finalInsert[i].date
                let timing = finalInsert[i].timing
                pool.query(`UPDATE ATTENDANCE_SEM4 SET STATUS = '${status}' WHERE studid = '${studid}' and cldate = '${date}' and ttid = ${timing} ;`, (error, results) => {
                    if (error) {
                        reject(error);
                    }
                    resolve('students attendance added');
